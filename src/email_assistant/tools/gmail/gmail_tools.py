...
try:
    ...
    from dateutil.parser import (
        parse as parse_time,  # pyright: ignore[reportMissingModuleSource]
    )
...
def mark_as_read(
    ...
    if not GMAIL_API_AVAILABLE:
        logger.warning("Gmail API not available, cannot mark message as read")
        return
...
def fetch_group_emails(
    ...
    if not use_mock and not gmail_token and not gmail_secret:
        ...
        if not os.path.exists(token_path) and not os.path.exists(secrets_path):
            logger.warning(
                "No Gmail API credentials found. Looking for token.json or secrets.json in .secrets directory"
            )
...
class SendEmailInput(BaseModel):
    ...
    additional_recipients: List[str] | None = Field(
